...
### 1.7.1使用MyBatis-plus快速开发

- 使用mp提供业务层通用接口```IService<T>```和业务层通用实现类```ServiceImpl<M,T>```
- 在通用类基础上做功能重载或功能追加
...
- **设计表现层返回结果的模型类，用于后端与前端进行数据统一，也称为前后端数据协议**
...
为什么要用AJAX，因为在此项技术之前，前端页面html是不能获取后端返回的数据的，是需要用jsp，在servlet返回数据后用jsp渲染，使用ajax后就可以替换jsp了，这样也可以实现前后端分离